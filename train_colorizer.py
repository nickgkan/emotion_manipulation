...
from train_test_utils import (
    load_from_ckpnt, clip_grad, back2color, unnormalize_imagenet_rgb, rgb_to_grayscale
)
...
def train_colorizer(model, data_loaders, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, ex in enumerate(data_loaders['train']):
            ...
            neg_samples = rgb_to_grayscale(images.clone().to(device))
            # negative samples
            ...
            if step % 50 != 0:
                continue
            writer.add_image(
                'ld/0_random_image_sample',
                back2color(unnormalize_imagenet_rgb(pos_samples[0])),
                epoch * len(data_loaders['train']) + step
            )
            writer.add_image(
                'ld/1_ld_start',
                back2color(unnormalize_imagenet_rgb(neg_list[0])),
                epoch * len(data_loaders['train']) + step
            )
            writer.add_image(
                'ld/2_ld_end',
                back2color(unnormalize_imagenet_rgb(neg_list[-1])),
                epoch * len(data_loaders['train']) + step
            )
            neg_list = [
                back2color(unnormalize_imagenet_rgb(neg))
                for neg in neg_list
            ]
        ...
        torch.save(
            ...
            "colorizer_%02d.pt" % (epoch+1)
        )
        print('\nValidation')
        print(eval_colorizer(model, data_loaders['test'], args))
    return model


@torch.no_grad()
def eval_colorizer(model, data_loader, args):
